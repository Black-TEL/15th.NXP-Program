...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTD_B0_IN, (void *)ov7725_eagle_img_buff, PTE2, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
    DMA_DIS(CAMERA_DMA_CH);
    disable_irq(PORTE_IRQn);                        //关闭PTA的中断
    DMA_IRQ_CLEAN(CAMERA_DMA_CH);                   //清除通道传输中断标志位
    DMA_IRQ_EN(CAMERA_DMA_CH);

    port_init(PTE2, ALT1 | DMA_FALLING | PULLUP );         //PCLK

    port_init(PTE3, ALT1 | IRQ_RISING | PULLUP | PF);     //场中断，上拉，上降沿触发中断，带滤波
